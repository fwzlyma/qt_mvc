        ...
        try:
            for root, dirs, files in os.walk(self.process_path, topdown=False):
                for name in files:
                    ...
                    cv2.imwrite(self.mulpicR_1,im)
                    # 处理图片----------------------------------------------------
                    imglas = Image.open(self.mulpicR_1)
                    height = imglas.size[1]
                    width = imglas.size[0]
                    x = int(600 * (height / width))
                    imglas = imglas.resize((600, x), Image.ANTIALIAS)
                    imglas.save(self.mulpicR_1)
                    # ************************************************************
                    ...
                    # print(new_name)
                    # cv2.imencode(".png", im_res)[1].tofile(self.save_path)
                    img_new_path = os.path.join(self.save_path, ".".join([new_name, "png"]))
                    cv2.imencode(".png", im_res)[1].tofile(img_new_path)
                # time.sleep(5000)
        except Exception as e:
            print(e)
        # finally:
